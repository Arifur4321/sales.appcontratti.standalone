    ...
    $(document).ready(function() {
        ...
        function checkSignatureStatus() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    console.log('Signature statuses:', response.statuses);

                    // Loop through each status in the response
                    for (const envelopeId in response.statuses) {
                        if (response.statuses[envelopeId] === 'signed' || response.statuses[envelopeId] === 'declined') {
                            // Stop the interval if any status is signed or declined
        ...
        // Initialize the interval on page load
        signatureCheckInterval = setInterval(checkSignatureStatus, 60000); // Check every 1 minute
